...
public class UserValidator implements Validator {
    ...
    public void validate(Object o, Errors errors) {
        ...
        if (user.getPassword().length() < 1 || user.getPassword().length() > 32) {//TODO
    ...
    public void valid(User user, Map<String, String> errors) {
        ...
        if (user.getUsername().length() < 1 || user.getUsername().length() > 32) {
            errors.put("username", "Size.userForm.username");
        }
        if (userService.findByUsername(user.getUsername()) != null) {
            errors.put("username", "Duplicate.userForm.username");
        }
        if (user.getPassword().length() < 1 || user.getPassword().length() > 32) {//TODO
            errors.put("password", "Size.userForm.password");
        }
        if (!user.getConfirmPassword().equals(user.getPassword())) {
            errors.put("confirmPassword", "Different.userForm.password");
        }
    }
}
